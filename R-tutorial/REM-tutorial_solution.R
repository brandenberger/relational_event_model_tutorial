...
##[1] "de_CH.UTF-8"
dtcont <- eventSequence(dt$date.event, dateformat = "%Y-%m-%d", 
                    data = dt, 
                    type = "continuous", 
                    excludeTypeOfDay = c('Samstag', 'Sonntag'),
                    excludeFrom = c('2010-12-24', '2011-12-24', '2012-12-24',
                                    '2013-12-24', '2014-12-24', '2015-12-24'),
                    excludeTo = c('2011-01-02', '2012-01-02', '2013-01-02',
                                  '2014-01-02', '2015-01-02', '2016-01-02'),
                    returnData = TRUE, 
                    sortData = TRUE)
table(is.na(dtcont$event.seq.cont))
dt$date.event[is.na(dtcont$event.seq.cont)]
## 5 events occurred on Saturday/Sunday! => be careful when excluding dates!

## "returnData=TRUE" will add the evente sequence variable to the data set and 
## hand the full data set back to you. This way you won't encounter any mistakes
## that may happen due to false sorting of your data frame.
...
## atEventTimesOnly = FALSE => now at each event a risk set is built!
## untilEventOccurrs = TRUE => now the risk set does not thin out towards the end!
dtremfuller <- createRemDataset(data = dt, 
                                sender = dt$chamber, 
                                target = dt$legnum,
                                eventSequence = dt$event.seq.cont, 
                                atEventTimesOnly = FALSE, 
                                untilEventOccurrs = TRUE,
                                returnInputData = TRUE)
...
## Task 4 - Inertia statistic
dtrem$inertia <- inertiaStat(data = dtrem, 
                             time = dtrem$eventTime, 
                             sender = dtrem$chamber, 
                             target = dtrem$legnum, 
                             eventvar = dtrem$eventDummy,
                             halflife = 10, 
                             showprogressbar = TRUE)
table(dtrem$inertia) #makes sense: no repeated events in this data frame by design!

## Task 5 - four-cycle statistic
...
## Task 6 - four-cycle statistic in parallel
# find out how many cores you have on your computer:
library(parallel)
detectCores()
# then run the statistic. in parallel you cannot use the progress bar, set it to FALSE.
dtrem$fourCycle <- fourCycleStat(data = dtrem, 
                                 time = dtrem$eventTime, 
                                 sender = dtrem$chamber, 
                                 target = dtrem$legnum, 
...
## Task 7 - for example: 
dtrem$parfam_homophily_hl1 <- degreeStat(dt, 
                                         time = dtrem$eventTime, 
                                         degreevar = dtrem$legnum, 
                                         halflife = 1, 
                                         eventtypevar = dtrem$parfam, 
                                         ...
                                         eventvar = dtrem$eventdummy, 
                                         showprogressbar = TRUE)
dtrem$parfam_homophily_hl150 <- degreeStat(dt, 
                                           time = dtrem$eventTime, 
                                           degreevar = dtrem$legnum, 
                                           halflife = 150, 
                                           eventtypevar = dtrem$parfam, 
...
## Task 8 - correlations: 
cor(cbind(dtrem$parfam_homophily, dtrem$entryRound_homophily, 
          dtrem$polSys_homophily, dtrem$fourCycle, dtrem$parfam_homophily_hl1))
## High correlations among the homophily statistics because they are all
## based on a degree statistic. HOWEVER! in the correlation, the 0-values are
## included as well and 
table(dtrem$parfam_homophily) 
## has 350 0-values (for instance) - they obviously correlate highly with each
## other => which inflates the correlation-coefficient. 
## In this case it's not that bad, but I've run rems where there was a lot of 
## zero-inflation going on and the correlations among the statistics is
## arbitrarily increased by it. Just something to keep an eye out for and a 
## reminder why it's always good to look at bivariate statistics. 
## If you have model terms that correlate highly, run the regression separatelly
## for both and see how the results behave! if the results change drastically 
## from the model with both variables in it, then I would exclude one of the
## variables!
...
## Task 9: 
xlog50 <- log(2)/50
baseline_50 <- exp(-(timediff)*xlog50) * xlog50
dtrem$parfam_homophily_rescaled <- dtrem$parfam_homophily_hl150/baseline_50
